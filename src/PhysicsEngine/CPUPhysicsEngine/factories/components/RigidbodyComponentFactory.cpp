...
bool RigidBodyComponentFactory::validateTransformComponent(const TransformComponent& transform) {
    for (int i = 0; i < 3; i++) {
        if (!std::isfinite(transform.position[i]) || !std::isfinite(transform.scale[i])) return false;
    }
    for (int i = 0; i < 4; i++) {
        if (!std::isfinite(transform.rotation[i])) return false;
    }
    if (transform.scale[0] <= 0.0f || transform.scale[1] <= 0.0f || transform.scale[2] <= 0.0f) return false;
    return true;
}

bool RigidBodyComponentFactory::validatePhysicsComponent(const PhysicsComponent& physics) {
    if (physics.mass < 0.0f || !std::isfinite(physics.mass)) return false;
    for (int i = 0; i < 3; i++) {
        if (!std::isfinite(physics.velocity[i]) || !std::isfinite(physics.angularVelocity[i])) return false;
    }
    if (physics.restitution < 0.0f || physics.restitution > 1.0f || physics.friction < 0.0f || physics.friction > 1.0f) return false;
    return true;
}

bool RigidBodyComponentFactory::validateBoxColliderComponent(const BoxColliderComponent& collider) {
    if (collider.width <= 0.0f || collider.height <= 0.0f || collider.depth <= 0.0f) return false;
    if (!std::isfinite(collider.width) || !std::isfinite(collider.height) || !std::isfinite(collider.depth)) return false;
    return true;
}

void RigidBodyComponentFactory::setTransformPosition(TransformComponent& transform, float x, float y, float z) {
    transform.position[0] = x; transform.position[1] = y; transform.position[2] = z;
}

void RigidBodyComponentFactory::setTransformScale(TransformComponent& transform, float sx, float sy, float sz) {
    transform.scale[0] = sx; transform.scale[1] = sy; transform.scale[2] = sz;
}

void RigidBodyComponentFactory::setPhysicsVelocity(PhysicsComponent& physics, float vx, float vy, float vz) {
    physics.velocity[0] = vx; physics.velocity[1] = vy; physics.velocity[2] = vz;
}

void RigidBodyComponentFactory::updatePhysicsMass(PhysicsComponent& physics, float mass) {
    physics.mass = mass; calculateInverseMass(physics);
}

void RigidBodyComponentFactory::initializeQuaternion(float* quaternion) {
    quaternion[0] = 1.0f; quaternion[1] = quaternion[2] = quaternion[3] = 0.0f;
}

void RigidBodyComponentFactory::calculateInverseMass(PhysicsComponent& physics) {
    physics.invMass = (physics.isStatic || physics.mass <= 0.0f) ? 0.0f : (1.0f / physics.mass);
}
